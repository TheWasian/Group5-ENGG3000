...
const SENSOR_API_BASE = window.location.hostname === '192.168.4.1'
    ? ''
    : 'http://192.168.4.1';
...
function whackHole(holeIndex, source = 'mouse') {
    ...
    if (hole !== activeHole || !hole.classList.contains('active')) {
        statusMessage.textContent = source === 'sensor'
...
function updateProgress() {
    ...
    const levelProgress = Math.max(0, Math.min(score - progressStart, progressEnd - progressStart));
...
function getPlayerPosition(data) {
    ...
    const validSensorIndexes = data.sensors
        .map((sensor, index) => sensor.valid && Number.isInteger(Number(sensor.hole)) ? index : -1)
    ...
    if (!Number.isInteger(holeIndex) || holeIndex < 0 || holeIndex >= holes.length) {
        return null;
    }
...
function displayPlayerPosition(data) {
    ...
    const targetHole = holes.find(hole => Number(hole.dataset.hole) === position.holeIndex);
...
function updateSensorStatus(connected, data = null) {
    ...
    if (!connected) {
        playerMarker.classList.add('hidden');
        updateDebugPanel();
        sensorStatus.textContent = 'Sensor controller: disconnected (mouse testing is available)';
        return;
    }
    ...
    const activeSensors = data.sensors
        .map((sensor, index) => sensor.valid ? `S${index + 1}: ${sensor.distance_cm} cm` : null)
        .filter(Boolean);
...
async function pollSensors() {
    ...
    try {
        const response = await fetch(`${SENSOR_API_BASE}/api/hits`, { cache: 'no-store' });
...
holes.forEach(hole => {
    hole.addEventListener('click', () => whackHole(Number(hole.dataset.hole), 'mouse'));
});
